...
%ifdef MODULE_8BIT_IDE
	; XT-CFv3 does not support largest block size in DMA mode,
	; so check for that device:
	cmp		BYTE [di+DPT_ATA.bDevice], DEVICE_8BIT_XTCF_DMA
	jne	.SetBlockSize
	; XT-CFv3 in DMA mode selected - check requested block size
	cmp		al, XTCF_DMA_MODE_MAX_BLOCK_SIZE
	jbe	.SetBlockSize
	; Otherwise, limit block size to maximum permitted
...
%endif ; MODULE_8BIT_IDE
	...
	pop		bx				; BL = Block size (preserve AH)
